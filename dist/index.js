...
async function safeChat(msgs, defs) {
    let attempts = 0;
    while (attempts < 3) {
        try {
...
`;
        ...
        const contextStats = tokenTracker.calculateContextStats(systemPrompt, historyToInclude, userText, toolDefs, currentModel);
        ...
        const parseInlineToolCalls = (msg) => {
            if (!msg.content)
                return msg;
        ...
        const updatedHistory = messages.filter((m, index) => {
            if (index === 0 && m.role === 'system')
                return false; // Skip the main system prompt
            return true;
        }).slice(-50);
        ...
        if (updatedHistory.length > 20 || historyTokens > 4000) {
            ...
            compressor.compressSession(sessionId, updatedHistory).catch((err) => console.error(`[COMPRESSOR] Error in background compression:`, err));
        }
    }
    catch (error) {
